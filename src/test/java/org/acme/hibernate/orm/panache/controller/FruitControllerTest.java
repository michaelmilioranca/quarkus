...
public class FruitControllerTest {
  ...
  public void testListAllFruits() {
    ...
    response =
        given()
            ...
            .extract()
            .response();
    assertThat(response.jsonPath().getList("name")).containsExactlyInAnyOrder("Apple", "Banana");
    ...
    response = given().when().get("/fruits").then().statusCode(200).extract().response();
    assertThat(response.jsonPath().getList("name"))
  ...
  public void testEntityNotFoundForDelete() {
    given().when().delete("/fruits/9236").then().statusCode(404).body(emptyString());
  }
